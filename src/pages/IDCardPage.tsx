/**
 * Tarjeta de Identificación Digital - Diseño estético modular funcional
 */

import React, { useState, useEffect } from 'react';
import { useVoiceNavigation } from '../shared/contexts/VoiceNavigationContext';
import { cardService } from '../features/identification-card/services/cardService';
...
const IDCardPage: React.FC = () => {
  ...
  const loadCard = async () => {
    try {
      ...
      if (data) {
        setCard(data);
        setFormData(data);
        speak('Tarjeta de identificación cargada');
      } else {
        speak('No hay tarjeta registrada. Crea una nueva');
      }
    } catch (error) {
      console.error('Error loading card:', error);
      speak('Error al cargar tarjeta', 'high');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombreCompleto.trim() || !formData.cedula.trim()) {
      speak('Completa nombre y cédula', 'high');
      return;
    }

    try {
      setLoading(true);
      await cardService.saveCard(formData, userId);
      await loadCard();
  ...
  const readCardInfo = () => {
    ...
    const text = `
      Tarjeta de identificación de ${card.nombreCompleto}.
      Cédula: ${card.cedula}.
      Tipo de sangre: ${card.tipoSangre}.
      Condiciones médicas: ${card.condicionesMedicas}.
      Contacto de emergencia: ${card.contactoEmergencia}, teléfono ${card.telefonoEmergencia}.
    `;
    speak(text, 'high');
  };
      ...
      <div className="space-y-4">
        {loading && !editing ? (
            ...
            <Card className="bg-gradient-to-br from-purple-600 to-purple-800 rounded-3xl p-6 shadow-2xl stagger-item">
              ...
              <div className="bg-white rounded-2xl p-4 flex flex-col items-center shadow-lg">
                ...
                <p className="text-purple-900 text-xs mt-2 text-center">
                  Muestra este código a conductores o personal médico
                </p>
              </div>
              ...
              {showFull && (
                <div className="mt-4 space-y-2 text-sm fade-in">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                    <p className="text-xs text-purple-200 mb-1">Condiciones Médicas</p>
                  ...
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 border border-white/20">
                    ...
                    <p className="text-purple-200">📞 {card.telefonoEmergencia}</p>
                  </div>
                </div>
              )}
          ...
          <Card className="stagger-item">
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-sm font-semibold text-purple-200 mb-2">Condiciones Médicas</label>
                <textarea
        ...
        <Card className="bg-yellow-900/30 border-yellow-500/30 stagger-item" style={{ animationDelay: '0.3s' }}>
          <p className="text-sm text-yellow-200 flex items-start gap-2">
            ...
            <span>Esta tarjeta contiene información médica importante. Asegúrate de mantenerla actualizada para emergencias.</span>
          </p>
        </Card>
      </div>
    </PageLayout>
